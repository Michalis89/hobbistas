...
export class RegisterDto {
  ...
  @IsString()
  @MinLength(8, {
    message: 'Το password πρέπει να έχει τουλάχιστον 8 χαρακτήρες',
  })
  @Matches(/((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/, {
  ...
  @Matches(/^[a-z0-9_-]+$/, {
    message:
      'Το όνομα χρήστη μπορεί να περιέχει μόνο πεζά γράμματα, αριθμούς, _ και -',
  })
  username: string;
  ...
  @MinLength(2, {
    message: 'Το display name πρέπει να έχει τουλάχιστον 2 χαρακτήρες',
  })
  displayName: string;
