-- ==============================================
-- HOBBISTAS DATABASE SCHEMA
-- Supabase PostgreSQL Migration
-- ==============================================
-- Τρέξε αυτό το αρχείο στο Supabase SQL Editor
-- ==============================================
...
CREATE TABLE public.categories (
  id VARCHAR(50) PRIMARY KEY, -- 'gaming', 'dnd', 'books', etc.
  label VARCHAR(100) NOT NULL,
  icon VARCHAR(10), -- emoji
  description TEXT,
  color VARCHAR(100), -- Tailwind gradient classes
...
CREATE TABLE public.articles (
  ...
  read_time_minutes INTEGER DEFAULT 5, -- Auto-calculated
...
CREATE TABLE public.comments (
  ...
  article_id UUID NOT NULL REFERENCES public.articles(id) ON DELETE CASCADE,
  author_id UUID NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,
  parent_id UUID REFERENCES public.comments(id) ON DELETE CASCADE, -- Για nested comments
...
CREATE TABLE public.likes (
  ...
  -- Constraint: Ένα like είτε σε article είτε σε comment
  CONSTRAINT like_target CHECK (
    (article_id IS NOT NULL AND comment_id IS NULL) OR
    (article_id IS NULL AND comment_id IS NOT NULL)
  ),

  -- Unique constraint: Ένας user μπορεί να κάνει like μόνο μία φορά
...
CREATE TABLE public.follows (
  ...
  -- Δεν μπορείς να follow τον εαυτό σου
...
CREATE TABLE public.notifications (
  ...
  actor_id UUID REFERENCES public.profiles(id) ON DELETE SET NULL, -- Ποιος έκανε την ενέργεια

  type VARCHAR(50) NOT NULL, -- 'comment', 'like', 'follow', 'mention', 'article_published', 'system'
...
SELECT
  ...
  -- Trending score: weighted combination of views, likes, comments
...
-- ==============================================
-- SEED DATA (Optional - για testing)
-- ==============================================
-- Αυτό μπορείς να το uncomment για να έχεις test data

/*
-- Insert test user profile (after auth.users is created via Supabase Auth)
-- You'll need to create the auth user first, then run this with the actual UUID
-- INSERT INTO public.profiles (id, display_name, username, email, roles)
-- VALUES
--   ('your-auth-user-uuid-here', 'Μιχάλης Καρκάνης', 'michalis', 'michalis@hobbistas.gr', ARRAY['admin', 'author']);
*/
...
-- Επόμενο βήμα: Τρέξε το NestJS backend
